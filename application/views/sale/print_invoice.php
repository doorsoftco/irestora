...
<p><?php echo $this->session->userdata('address'); ?>
    ...
    ?><br>
    <br>
    <?=isset($info->token_no) && $info->token_no?"Token No: ".$info->token_no:''?>
    <br>
...
Sales Associate: <?php echo $info->full_name; ?></p>
    <p>Customer: <?php echo "$info->customer_name"; ?><?=isset($info->table_name) && $info->table_name?" Table No: ".$info->table_name:''?></p>
    ...
    foreach ($details as $row) {
        ...
        <tr>
            <td class="no-border border-bottom">(CODE:<?php echo $row->code; ?>)
                <small></small> <?php echo "$row->qty X $row->price"; ?> </td>
